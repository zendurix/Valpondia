...
std::vector<SharedPtr<Room>> BspMapGen::make_dung_map_ret_rooms(Level& level, int treeHeight)
{
	BspMapGen tree(level.get_fieldRef(), treeHeight);
	tree.levelPtr = &level;
...
SharedPtr<Room> BspMapGen::make_random_room_in_node(int nodeIndex)
{
	...
	//do
	{
		length = RNG::randInt(MIN_ROOM_SIZE, maxLength);
		height = RNG::randInt(MIN_ROOM_SIZE, maxHeight);
	...
	if (parentX + PLACE_FOR_WALLS < maxX)	x = RNG::randInt(parentX, maxX);
	else									x = parentX;

	if (parentY + PLACE_FOR_WALLS < maxY)	y = RNG::randInt(parentY, maxY);
	else									y = parentY;
...
void BspMapGen::connect_all_rooms()
{
	...
	for (int treeLevel = treeHeight; treeLevel > 0; treeLevel--)
	{
		...
		for (int i = startNodeIndex; i < maxNodeIndex; i += iAdder())
		{
			...
			int doorPosY1 = RNG::randInt(node1->room->cornerNE.y + 2, node1->room->cornerNE.y + 2 + (node1->room->height - 2));
			int doorPosY2 = RNG::randInt(node2->room->cornerNE.y + 2, node2->room->cornerNE.y + 2 + (node2->room->height - 2));
			int doorPosX1 = RNG::randInt(node1->room->cornerSW.x + 2, node1->room->cornerSW.x + 2 + (node1->room->length - 2));
			int doorPosX2 = RNG::randInt(node2->room->cornerNW.x + 2, node2->room->cornerNW.x + 2 + (node2->room->length - 2));
...
#define CHECKmax (searcher1->get_x() != MYLENGTH - 1 && searcher1->get_y() != MYHEIGHT - 1)
	...
	while (searcher1 != searcher2)
	{
		...
		for (int i = 1; i < 10; i++)
		{
			...
			if (ignoreIsWall || !searcher1->get_isObstacle() || searcher1 == searcher2)
			{
...
void BspMapGen::add_stairs_down(std::vector<SharedPtr<Room>> rooms)
{
	SharedPtr<Room> roomWithStairs = RNG::random_from_vect(rooms);
	SharedPtr<Place> stairsDown;
	do
	{
		stairsDown = RNG::random_from_vect(roomWithStairs->inRoom);
	} while (!stairsDown->is_in_room());

	stairsDown->set_connector(Connector{ stairsDown->get_y(), stairsDown->get_x(), stairsDown->get_levelID() + 1, '-', true, false });

	levelPtr->set_stairsDown(stairsDown);
}