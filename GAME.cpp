...
SharedPtr<Place> GAME::get_place(Position pos) 
{ 
	return levels[pos.levelID]->get_fieldRef()[pos.y][pos.x]; 
}
...
void GAME::init()
{
	...
	Place::set_textures(&Printer->textures);
...
Entity GAME::make_player()
{
	...
	ecs->add_component(newPlayer, Vision{ 90 });
...
void GAME::make_level(Connector connector, bool makeAllLevels)
{
	...
	auto makeLevel2 = [&]()
	{
		...
		newLevel = new Level(levelType, 1, nullptr, "", &levels[0]->get_roomsRef());
	...
	auto makeLevel3 = [&]()
	{
		...
		LvType levelType = LEVEL_ORDER[3].first;
		newLevel = new Level(levelType, 2, levels[2]->stairsDown);
		
		newLevel->get_stairsUp()->set_connector(
			Connector{ levels[2]->get_stairsDown()->get_y(), levels[2]->get_stairsDown()->get_x(), 2, '+',  false, true });
		//levels[2]->get_stairsDown()->set_connector(
	...
	auto makeLevel4 = [&]()
	{
		LvType levelType = LEVEL_ORDER[4].first;
		newLevel = new Level(levelType, 3, levels[3]->stairsDown);

		newLevel->get_stairsUp()->set_connector(
			Connector{ levels[3]->get_stairsDown()->get_y(), levels[3]->get_stairsDown()->get_x(), 3, '+',  false, true });
	};
