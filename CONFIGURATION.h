...
// game modes:
#define GAME_MODE_ASCII 1


enum class LvType { cave = 0, dungeon = 1, fromFile = 2, ruins, basements };
...
const std::vector<lvPair> LEVEL_ORDER = { lvPair(LvType::ruins, ""), lvPair(LvType::fromFile, "valpondia_palace.txt"),
					lvPair(LvType::basements, ""), lvPair(LvType::dungeon, ""), lvPair(LvType::cave, ""), lvPair(LvType::cave, "") };



enum class TextureType { rockWall = 0, floorTile, brickWall, player, sword, rat, junkScarab };
...
const int UI_LENGTH = 400;  // length from right border

#if GAME_MODE_ASCII == 1
	const int TILE_HEIGHT = 16; // in pixels
	const int TILE_LENGTH = 16; // in pixels
#else
	const int TILE_HEIGHT = 32; // in pixels
	const int TILE_LENGTH = 32; // in pixels
#endif
...
const int WIN_LENGTH = 1600;

const int HEIGHT_TILES = WIN_HEIGHT / (TILE_HEIGHT + TILE_GAP);
const int LENGTH_TILES = (WIN_LENGTH - UI_LENGTH) / (TILE_LENGTH + TILE_GAP);
...
// RUINS MAP
const int RUINS_MIN = 11;
const int RUINS_MAX = 13;
//
