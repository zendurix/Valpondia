...
void Move::update(Entity entity)
{
	...
	if (moveEvent != NULL_EVENT)
	{
		...
		if (!border_cross(pos))
		{
			if (gamePtr->get_levelActive()->get_fieldRef()[pos.y][pos.x]->get_isObstacle())
				pos = prev;
		}
		else if (gamePtr->get_levelActive()->get_fieldRef()[pos.y][pos.x]->get_entityCharHere() != MAX_ENTITIES)
		{
			eventManager->send_event(Event{ EventType::doAttack, static_cast<int>(gamePtr->get_levelActive()->get_fieldRef()[pos.y][pos.x]->get_entityCharHere()) },
				entity);
			pos = prev;
		}
		else
		...
		gamePtr->get_levelActive()->get_fieldRef()[pos.y][pos.x]->go_here(entity);
	}
}
